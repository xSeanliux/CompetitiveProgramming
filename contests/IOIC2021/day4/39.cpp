...
struct Cost_flow{
	...
	void add_edge(int from, int to, int cap, int cost){
		G[from].push_back(Edge(to, cap, (int)G[to].size(), cost)); //why no -1?
...
int main(){
	...
	for(int i = 1; i <= N; i++){
		cin >> w;
		flow.add_edge(i, i + N, w, 0);
	}
	for(int i = 0; i < M; i++){
		cin >> u >> v >> c >> f;
		flow.add_edge(u + N, v, c, f);
	}
	flow.add_edge(0, 1, K, 0);
	flow.add_edge(2 * N, 2 * N + 1, K, 0);
	pair<int,int> ans = flow.flow();
